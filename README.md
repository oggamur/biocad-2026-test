...
Проект может быть развернут на GitHub Pages как статический сайт. Подробные инструкции см. в файле [DEPLOY.md](./DEPLOY.md).

### Быстрый старт для GitHub Pages:

1. **Создайте репозиторий на GitHub** (если еще не создан)

2. **Настройте автоматический деплой** (рекомендуется):
   - Перейдите в **Settings** → **Pages** вашего репозитория
   - В разделе **Source** выберите **GitHub Actions**
   - При каждом push в ветку `main` или `master` будет автоматически запускаться деплой
   - Workflow автоматически сгенерирует данные, скомпилирует CSS и задеплоит сайт

3. **Или ручной деплой**:
   ```bash
   npm run build  # Генерирует данные и компилирует CSS
   git add .
   git commit -m "Prepare for GitHub Pages"
   git push
   ```
   Затем в настройках репозитория выберите ветку и папку `/public`

**Важно:** 
- GitHub Pages поддерживает только статические файлы. Node.js сервер не работает на GitHub Pages.
- Код автоматически адаптируется для работы со статическими JSON файлами.
- Для работы с полноценным Node.js сервером используйте альтернативные платформы (Vercel, Netlify, Railway и т.д.).
